...
class Responsive_Slides {
    ...
    public function responsive_slideshow_config() {
        ...
        if (isSet($json_decoded)) {
            echo esc_attr($json_decoded->speed);
        }else{
            echo esc_attr(500);
        }
        ?>" />
            ...
            <tr valign="top">
                ...
                <td><input type="text" name="timeout" value="<?php
                   if (isSet($json_decoded)) {
                       echo esc_attr($json_decoded->timeout);
                   }else{
                       echo esc_attr(4000);
                   }
        ?>" />
        ...
        ?>>true</option>
            ...
            <tr valign="top">
                ...
                <td><input type="text" name="namespace" value="<?php
                        if (isSet($json_decoded)) {
                            echo esc_attr($json_decoded->namespace);
                        }else{
                            echo esc_attr('callbacks');
                        }
        ?>" />
    ...
    public function responsive_save_post($post) {
        ...
        if (isset($post)) {
            if (!wp_is_post_revision($post->ID)) {
                if (isset($img)) {
                    update_post_meta($post->ID, 'responsive_slide_' . $post->ID, esc_attr($img));
    ...
    public function responsive_image_upload_metabox($post) {
        ...
        ?><input type="hidden" name="images" id="images" value="<?php echo esc_attr($svalue); ?>" />
    ...
    public function responsive_side_shortcode_metabox($post) {
        global $post;
        $messsage = "<label>Your Shortcode is:<br> [responsive_slide id=".esc_attr($post->ID)."]</label>";
        echo $messsage;
    }
    ...
    public function responsive_render_slideshow($images) {
        ...
        foreach ($images as $img) {
            $output .= '<li><img src="' . esc_attr($img) . '" /></li>';
        }
        $output .= '</ul>';
        $output .= '</div>';
        return $output;
    ...
    public function responsive_image_column_content($column_name, $postID) {
        ...
        if ($this->responsive_is_page_responsive_post() == TRUE) {
            ...
            if ($column_name == 'shortcode_key' &&
                    get_post_status($postID) != 'draft') {
                $output = '[responsive_slide id=' . esc_attr($postID) . ']';
                echo $output;
            }
        }
    }
